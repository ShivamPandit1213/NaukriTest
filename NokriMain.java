...
//		PublicMethod call = new PublicMethod(driver);
		...
		while (true) {
			...
			// Perform your job scraping logic here on the current page

			// Check if "Next" is enabled and clickable
			while (true) {
				System.out.println("\n___________Currently on Page: " + pageIndex+"___________");

				// System.out.println("\nTotal page count in pagination: " + pageCount);//
...
//			System.out.println("\nTotal job count: " + countJob.size() + ", on current page: " + currPageName);
					...
					try {
						...
						if (jobButtonName.equalsIgnoreCase("Apply on company site")) {
							...
							if (checkJobType.isEnabled() || !checkJobType.isEnabled()) {
								...
								checkJobType.click();
								Thread.sleep(5000);
								List<WebElement> chatbotClose = driver.findElements(By
										.xpath("//div[contains(@class,'chatbot')]//button[contains(@class,'close')]"));
				...
				if (!nextPageButton.isEmpty() && nextPageButton.get(0).isDisplayed()
						&& nextPageButton.get(0).isEnabled()) {
